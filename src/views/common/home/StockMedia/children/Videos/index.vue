<template>
  <div class="Gifs">
    ...
    <div class="fromTitle">
      ...
      <img
        src="@/assets/img/stockmedia/pixabay.svg"
        alt=""
      >
    </div>
    <!-- hot部分 -->
            ...
            <div @click.stop="singleDownloadClick(item)">
              ...
              >
                <div class="popoverBox">
                  Shot by {{item.userName}} On Pixabay
                </div>
                <img
                  v-if="!item.downLoad"
                  class="posTop"
...
<script>
import { videosSearch, photosTrending } from '@/api/other'
  ...
  methods: {
    ...
    async searchStickers () {
      try {
        ...
        let res = await videosSearch({
          key: '26745843-4d6979c26b1803727b43e46df',
        ...
        if (res.status === 200 && res.data.msg === 'OK') {
          let resData = _.cloneDeep(res.data.data.hits).map(item => {
            item = {
              active: false,
              downLoad: false,
              s_id: item.id,
              preview_url: item.userImageURL,
              file_url: item.videos.tiny.url,
              ready: true,
              error: false
            }
            return item
          ...
          this.searchOffset = this.searchOffset + res.data.data.hits.length
          ...
          this.$nextTick(() => {
            resData.forEach(item => {
              this.eventTester('progress', item.s_id, item)
              this.eventTester('error', item.s_id, item)
            })
          })
        }
      }
    ...
    // 无限滚动
    async Searchload () {
      if (this.searchList.length > 500) return
        ...
        if (res.status === 200 && res.data.msg === 'OK') {
          let resData = _.cloneDeep(res.data.data.hits).map(item => {
            item = {
              ...
              downLoad: false,
              s_id: item.id,
              preview_url: item.videos.tiny.url,
              file_url: item.videos.large.url,
              ready: true,
              error: false,
              userName:item.user
            }
          ...
          this.Hotoffset = this.Hotoffset + res.data.data.hits.length
    ...
    eventTester (event, ref, item) {
      console.log(event, ref, this.$refs);
      this.$refs[ref][0].addEventListener(event, function () {
        if (event === 'progress') {
          item.ready = false
        }
        if (event === 'error') {
    ...
    async Hotload () {
      ...
      if (this.HotList.length > 500) return
      if (this.HotScrollloading) return
      this.HotScrollloading = true
